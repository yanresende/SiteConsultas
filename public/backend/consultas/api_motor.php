...
if (!isset($input['motor'])) {
    http_response_code(400);
    echo json_encode(['erro' => 'Motor não informado.']);
    exit;
}

$motor = strtoupper(preg_replace('/[^A-Z0-9]/', '', $input['motor']));
if (strlen($motor) < 13) {
    http_response_code(400);
    echo json_encode(['erro' => 'Motor inválido.']);
    exit;
}
...
$url = "https://consultafacil.pro/api/engine/{$motor}?token={$token}";
$ch = curl_init($url);
