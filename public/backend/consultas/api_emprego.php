...
if (!isset($input['emprego'])) {
    http_response_code(400);
    echo json_encode(['erro' => 'Cpf não informado.']);
    exit;
}

$emprego = trim($input['emprego']); // Limpa espaços em branco, mas mantém o cpf intacto
...
$url = "data.workbuscas.com/api/v1/{$token}/empregos/{$emprego}";
$ch = curl_init($url);
