...
// Lê e valida o titulo
$input = json_decode(file_get_contents('php://input'), true);
if (!isset($input['titulo'])) {
    http_response_code(400);
    echo json_encode(['erro' => 'titulo não informado.']);
    exit;
}

$titulo = trim($input['titulo']); // Limpa espaços em branco, mas mantém o titulo intacto
...
$url = "data.workbuscas.com/api/v1/{$token}/titulo/{$titulo}";
$ch = curl_init($url);
